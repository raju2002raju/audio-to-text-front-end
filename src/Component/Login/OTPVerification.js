...
const OTPVerification = () => {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await fetch('https://audio-to-text-back-end.onrender.com/api/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <form onSubmit={handleSubmit}>
      ...
      <p className="resend-text">
        ...
        <span
          ...
          style={{ cursor: isResending ? "not-allowed" : "pointer", color: isResending ? "gray" : "#FFBB00" }}
        >
